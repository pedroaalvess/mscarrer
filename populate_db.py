...
def populate_jobs():
    """Populate database with sample job postings in French"""
    ...
    with app.app_context():
        # Only add jobs if we have less than 6 jobs (preserve existing data)
        ...
        if existing_count < 6:
            ...
            print(f"✅ {new_count - existing_count} nouvelles offres d'emploi ajoutées!")
            print(f"📊 Total: {new_count} offres d'emploi disponibles")
        else:
            print(f"✅ Database already has {existing_count} jobs - no changes needed")
